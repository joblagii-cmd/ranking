// api/jobs-all.js - All jobs across all dates
...
async function getAllJobs(owner, repo, token, page = 1, limit = 24, search = "", industry = "", dateFilter = "") {
  try {
    ...
    const rootRes = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/contents/jobs`, {
      headers: { Authorization: `token ${token}`, Accept: "application/vnd.github.v3+json", "User-Agent": "JobPoster-Bot/1.0" }
    });
    if (!rootRes.ok) return { jobs: [], total: 0, dates: [] };
    const root = await rootRes.json();
    if (!Array.isArray(root)) return { jobs: [], total: 0, dates: [] };
    ...
    const targetDates = dateFilter ? dateFolders.filter(f => f.name === dateFilter) : dateFolders;

    // For each date, get company folders (but only fetch one date at a time to avoid timeout)
    const companiesPerPage = Math.ceil(limit / 5);
    let allCompanyFolders = [];

    for (const dateFolder of targetDates) {
      const foldersRes = await fetch(`${GITHUB_API}/repos/${owner}/${repo}/contents/jobs/${dateFolder.name}`, {
        headers: { Authorization: `token ${token}`, Accept: "application/vnd.github.v3+json", "User-Agent": "JobPoster-Bot/1.0" }
      });
      ...
      folders.filter(f => f.type === "dir").forEach(f => {
        allCompanyFolders.push({ ...f, date: dateFolder.name });
      });
    }

    const total = allCompanyFolders.length * 5;
    const startIdx = (page - 1) * companiesPerPage;
    const pageCompanies = allCompanyFolders.slice(startIdx, startIdx + companiesPerPage);

    if (pageCompanies.length === 0) return { jobs: [], total, dates };

    // Fetch files for this page's companies only
    const files = [];
    await Promise.all(pageCompanies.map(async folder => {
      try {
        ...
        data.forEach(f => { if (f.name.endsWith(".json") && f.type === "file") files.push({ ...f, date: folder.date }); });
      } catch { }
    }));
    ...
    const jobs = await Promise.all(files.map(async f => {
      try {
        ...
        return {
          ...
          skills: job.skills?.slice(0, 4) || [], path: f.path, date: f.date
        };
      } catch { return null; }
    }));
...
function renderHTML(jobs, total, dates, page, search, industry, dateFilter) {
  ...
  const dateOptions = dates.map(d => `<option value="${d}" ${dateFilter===d?"selected":""}>${d}</option>`).join("");
...
<meta name="description" content="Browse all job postings in India. Filter by industry, date, or search by role and company.">
<style>
...
.page-hero h1{font-size:2rem;font-weight:800;margin-bottom:0.4rem}.page-hero h1 span{color:#38BDF8}
...
@media(max-width:600px){.page-hero h1{font-size:1.5rem}.jobs-grid{grid-template-columns:1fr}.filter-row{flex-direction:column}.filter-row input,.filter-row select,.filter-row button{width:100%}}
</style>
</head>
<body>
<header class="header">
...
<div class="filters">
  <form class="filter-row" action="/jobs" method="get">
    ...
    <select name="date"><option value="">All Dates</option>${dateOptions}</select>
    <button type="submit">Filter</button>
  </form>
</div>
<div class="main">
  <div class="results-header">
    <div class="results-count">${total.toLocaleString()} jobs <span>${search?`for "${search}"`:""}${industry?` in ${industry}`:""}${dateFilter?` on ${dateFilter}`:""}</span></div>
  </div>
  ${jobs.length>0?`<div class="jobs-grid">${jobCards}</div>${pagination}`:`<div class="no-jobs"><h3>No jobs found</h3><p>Try adjusting your filters</p></div>`}
</div>
<footer class="footer">
