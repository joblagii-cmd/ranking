...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      apiError = JSON.stringify(data).substring(0, 200);
    }
  } catch (e) {
    apiError = e.message;
  }

  // Fallback: if API failed, generate last 7 days
  if (dates.length === 0) {
    for (let i = 0; i < 7; i++) {
...
<!-- dates found: ${dates.join(", ")} | error: ${apiError || "none"} | owner: ${GITHUB_OWNER} | repo: ${GITHUB_REPO} | token: ${GITHUB_TOKEN ? "set" : "MISSING"} -->
