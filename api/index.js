// api/index.js - Homepage with job listings (shows today, falls back to yesterday)
...
async function getJobsForDate(owner, repo, token, date, page = 1, limit = 20, search = "", industry = "") {
  try {
    const folders = await getFolderContents(owner, repo, token, `jobs/${date}`);
    if (!folders) return null; // date folder doesn't exist
    ...
    const allFiles = [];
    await Promise.all(companyFolders.map(async folder => {
      try {
        const files = await getFolderContents(owner, repo, token, folder.path);
        if (!files) return;
        files.forEach(f => {
          if (f.name.endsWith(".json") && f.type === "file") {
            allFiles.push({ path: f.path, download_url: f.download_url });
          }
        });
      } catch { }
    }));
    ...
    const jobs = await Promise.all(pageFiles.map(async f => {
      try {
        ...
        return {
          ...
          datePosted: job.datePosted, skills: job.skills?.slice(0, 4) || [], path: f.path
        };
      } catch { return null; }
    }));

    return { jobs: jobs.filter(Boolean), total, date };
  } catch (e) {
    return null;
  }
}
...
function renderHTML(jobs, total, displayDate, actualDate, page, search, industry, isFallback) {
  ...
  const fallbackBanner = isFallback ? `
    <div class="fallback-banner">
      ⏳ Today's jobs are being generated — showing latest posts from ${actualDate}. Fresh jobs will appear shortly!
    </div>` : "";
...
<div class="main">
  ...
  ${jobs.length > 0 ? `<div class="jobs-grid">${jobCards}</div>${pagination}` : `
    <div class="no-jobs"><h3>No jobs found</h3><p>Try a different search term or industry filter</p></div>
  `}
</div>
<footer class="footer">
...
export default async function handler(req, res) {
  ...
  res.setHeader("Cache-Control", "s-maxage=120"); // 2 min cache so fallback refreshes quickly
